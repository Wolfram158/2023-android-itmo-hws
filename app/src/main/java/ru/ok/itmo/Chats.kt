...
class Chats(private val channel: String) : Fragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        val channelView = view.findViewById<ChannelCustomView>(R.id.channel_custom_view)
        val nameOfChannel: TextView = view.findViewById(R.id.nameOfChannel)
        nameOfChannel.text = channel
        ...
        CoroutineScope(Dispatchers.IO).launch {
            ...
            if (messages.body()?.isNotEmpty()!!) {
                channelView.setInitials(messages.body()?.get(0)?.from)
            }
